...
import { selectFilterP } from "@/app/features/filter/FilterSelector";
...
import {
  ...
  clearSelectFilters,
  setOrderPrice,
} from "@/app/features/selectedFilter/selectedFilterSlice";
import {
  selectedFiltersG,
  selectHasFilters,
} from "@/app/features/selectedFilter/selectedFilterSelector";
...
export default function FilterBar({ onClose }: MoldeProps) {
  ...
  const Filtros = useSelector(selectFilterP);
  ...
  useEffect(() => {
    if (SelectedFilters.preciomin !== 0) {
      setPmin(SelectedFilters.preciomin.toString());
    }
    if (SelectedFilters.preciomax !== 1000) {
      setPmax(SelectedFilters.preciomax.toString());
    }
  }, [SelectedFilters]);

  const handerOrder = (seleted: optionsType | null) => {
    if (seleted) {
      dispatch(setOrderPrice({ valor: seleted?.value }));
      dispatch(Reset_Page());
    }
    onClose();
  };
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center">
      ...
      {HadFilters && (
        <div className="w-full flex flex-wrap justify-end items-center px-4">
          {" "}
          <button
            className="p-2 mb-4 ml-auto bg-white text-black rounded"
            onClick={() => {
              dispatch(clearSelectFilters(), setPmin(""), setPmax(""));
              onClose();
              window.scrollTo(0, 0);
              router.push("/");
              dispatch(Reset_Page());
      ...
      <div className="flex flex-warp gap-2 items-center justify-between mb-8 w-full text-l">
        <label>Ordenar precios:</label>
        <Select
          options={options}
          className="text-black ml-auto w-50"
          ...
          defaultValue={
            SelectedFilters.orderPrice === null
              ? null
              : options[SelectedFilters.orderPrice]
          }
    ...
    <div className="flex flex-col w-full items-center justify-center border-b-2 mb-4 pb-4">
      <div
      ...
      {category && (
        <div className="flex flex-col justify-start w-full my-4 text-xl">
        
          {Filtros.category.map((product) => (
            <label key={product.name} htmlFor={product.name}>
              <div className="flex flex-wrap justify-between items-center p-2 pl-8 border-b border-gray-400">
                ...
                <input
                  ...
                  onChange={(e) => {
                    dispatch(
                      e.target.checked
                        ? addSelectFilters({ category: product.name })
                        : deleteSelectFilters({
                            name: "category",
                            value: product.name,
                          }),
      ...
      <div className="flex flex-col w-full items-center justify-center border-b-2 mb-4 pb-4">
        ...
        >
          <h1 className="font-bold text-xl">Colores</h1>
                ...
                <div className="flex flex-wrap justify-between items-center p-2 pl-8 border-b border-gray-400">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      dispatch(
                        e.target.checked
                          ? addSelectFilters({ color: product.name })
                          : deleteSelectFilters({
                              name: "color",
                              value: product.name,
                            }),
     ...
     <div className="flex flex-col w-full items-center justify-center border-b-2 mb-4 pb-4">
        <div
          className="flex flex-wrap w-full items-center justify-between  "
          onClick={() => setTalla(!talla)}
        >
          <h1 className="font-bold text-xl">Tallas</h1>
                ...
                <div className="flex flex-wrap justify-between items-center p-2 pl-8 border-b border-gray-400">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      dispatch(
                        e.target.checked
                          ? addSelectFilters({ talla: product.name })
                          : deleteSelectFilters({
                              name: "talla",
                              value: product.name,
                            }),
          ...
          <div className="flex flex-wrap justify-center gap-4 w-full mt-4">
            <div className="flex flex-wrap gap-2">
              ...
              <input
                ...
                className="border-2 w-15 text-center"
              />
              <button
                onClick={() => {
                  dispatch(addSelectFilters({ preciomin: Number(pmin) }));
                  window.scrollTo(0, 0);
                  dispatch(Reset_Page());
                  router.push("/");
                }}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => {
                  dispatch(addSelectFilters({ preciomax: Number(pmax) }));
                  window.scrollTo(0, 0);
                  dispatch(Reset_Page());
                  router.push("/");
                }}
      ...
      <div
        className="bg-white text-black rounded p-4 flex flex-col items-center justify-center my-20"
        onClick={() => onClose()}
      >
